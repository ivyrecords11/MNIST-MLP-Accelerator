...
module gemv_adder_tree#(
    ...
    wire    signed  [W+L-1:0]   add_tmp     [L:0]   [N-1:0] ;
    ...
    generate
        ...
        for (b = 1; b <= L; b = b + 1) begin : TREE_LEVEL
            (* use_dsp   = "no", MULT_STYLE = "LUT" *)
            // compute how many nodes at this level
            localparam integer WIDTH_B = (N + (1<<b) - 1) >> b;
            for (d = 0; d < WIDTH_B; d = d + 1) begin
                assign add_tmp[b][d] = level_reg[b-1][2*d] + level_reg[b-1][2*d + 1];
            end
