...
module loc_ctrl3#(
    ...
    reg [$clog2(CYCLES+L2_TREE_DELAY):0] cnt;
    ...
    always @(posedge clk_i) begin
        ...
        if (clear_i) begin
            ...
            case (present_state)
                ...
                RUN: begin
                    if          (cnt >= 0 && cnt < L2_TREE_DELAY) begin
                        done    <= 0;
                        w_en    <= 0;   w_addr  <= 0;
                        cnt     <= cnt + 1;
                    end else if (cnt == L2_TREE_DELAY) begin
                        done    <= 0;
                        w_en    <= 1;   w_addr      <= 0;
                        save_en <= 0;
                        cnt     <= cnt+1;
                    end else if (cnt > L2_TREE_DELAY && cnt < CYCLES + L2_TREE_DELAY) begin
                        done    <= 0;
                        w_en    <= 1;   w_addr      <= w_addr+1;
                        save_en <= 0;
                        cnt     <= cnt+1;
                    end else if (cnt == CYCLES + L2_TREE_DELAY) begin
                        done    <= 1;
